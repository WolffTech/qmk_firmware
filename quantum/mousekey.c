...
#    endif /* #ifdef MK_KINETIC_SPEED */
    ...
    else if (code == MS_LEFT)
        mouse_report.x = move_unit() * -1;
    else if (code == MS_RGHT)
        mouse_report.x = move_unit();
    else if (code == MS_WHLU)
        mouse_report.v = wheel_unit();
    else if (code == MS_WHLD)
        mouse_report.v = wheel_unit() * -1;
    else if (code == MS_WHLL)
        mouse_report.h = wheel_unit() * -1;
    else if (code == MS_WHLR)
        mouse_report.h = wheel_unit();
    else if (IS_MOUSEKEY_BUTTON(code))
        mouse_report.buttons |= 1 << (code - MS_BTN1);
    else if (code == MS_ACL0)
        mousekey_accel |= (1 << 0);
    else if (code == MS_ACL1)
        mousekey_accel |= (1 << 1);
    else if (code == MS_ACL2)
        mousekey_accel |= (1 << 2);
}

void mousekey_off(uint8_t code) {
    if (code == MS_UP && mouse_report.y < 0)
        mouse_report.y = 0;
    else if (code == MS_DOWN && mouse_report.y > 0)
        mouse_report.y = 0;
    else if (code == MS_LEFT && mouse_report.x < 0)
        mouse_report.x = 0;
    else if (code == MS_RGHT && mouse_report.x > 0)
        mouse_report.x = 0;
    else if (code == MS_WHLU && mouse_report.v > 0)
        mouse_report.v = 0;
    else if (code == MS_WHLD && mouse_report.v < 0)
        mouse_report.v = 0;
    else if (code == MS_WHLL && mouse_report.h < 0)
        mouse_report.h = 0;
    else if (code == MS_WHLR && mouse_report.h > 0)
        mouse_report.h = 0;
    else if (IS_MOUSEKEY_BUTTON(code))
        mouse_report.buttons &= ~(1 << (code - MS_BTN1));
    else if (code == MS_ACL0)
...
#    endif
    if (code == MS_UP && mouse_report.y < 0)
        ...
        mouse_report.y = 0;
    else if (code == MS_LEFTr && mouse_report.x < 0)
        mouse_report.x = 0;
    else if (code == MS_RGHT && mouse_report.x > 0)
        mouse_report.x = 0;
    else if (code == MS_WHLU && mouse_report.v > 0)
